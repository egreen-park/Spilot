...
class LongitudinalMpc:
  def __init__(self, e2e=False):
    ...
    self.lo_timer = 0
    self.log = Loger()
    self.xStopFilter = StreamingMovingAverage(11)  #11
    self.xStopFilter2 = StreamingMovingAverage(3) #3
    self.xStop = 0.0
  ...
  def update(self, carstate, radarstate, model, controls, v_cruise, x, v, a, j, y, prev_accel_constraint):
    ...
    stopSign = v_ego_kph < 80.0 and model_x < 130.0 and ((v[-1] < 3.0) or (v[-1] < v[0]*0.70)) and abs(y[N]) < 3.0
    ...
    #apilot
    # stopFilterX = self.xStopFilter.process(model_x, median = True)  
    # self.xStop = self.xStopFilter2.process(stopFilterX)
    self.xStop = self.xStopFilter.process(model_x, median = True)  
    
    stopline3 = self.xStop * np.ones(N+1) + 5 # + self.trafficStopDistanceAdjust  
    ...
    self.stop_line_x_offset = interp(v_ego, [9.0, 15.0, 18.0, 20.0], [0.5, 0.0, -0.5, -1.0])

    stopline3 += self.stop_line_x_offset 

    stopping = self.stop_line and self.trafficState == 1 and not self.status and not carstate.brakePressed and not carstate.gasPressed
